...
FROM employees;
-- 커미션까지 포함한 최종 급여
SELECT first_name, salary, commission_pct, salary+salary*commission_pct 최종급여
FROM employees;
...
-- 입사일이 2001-01-01 이후인 사원들의 이름과 입사일을 출력
SELECT CONCAT(first_name,' ',last_name) AS name, hire_date AS 입사일 
FROM employees
...
SELECT CONCAT(first_name,' ',last_name) AS name, salary
FROM employees
WHERE 10000 <=salary AND salary <=17000
ORDER BY salary;
...
FROM employees
WHERE commission_pct IS NULL;	--   IS NULL을  =NULL로 하면 안됨
...
SELECT CAST("123" AS UNSIGNED);	-- 문자열 -> unsigned integer
